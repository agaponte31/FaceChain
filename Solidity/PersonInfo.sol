...
contract PersonInfo {
    ...
    mapping(bytes32 => uint) idToPersonIndex; // Nuevo mapeo

    constructor() {
            persons.push(Person(bytes32(0), bytes32(0), bytes32(0), 'none', false));
            
        } 

    // Función para añadir una persona (modificada para actualizar el mapeo)
    function addPerson(bytes32 _name, bytes32 _lastName, bytes32 _idNumber, string memory _fileCID) public {
    ...
    function getPersonByIdNumber(bytes32 _idNumber) public view returns (bytes32, bytes32, bytes32, string memory, bool) {
    ...
    function getPersonByIndex(uint _index) public view returns (bytes32, bytes32, bytes32, string memory, bool) {
    ...
    function updatePerson(bytes32 _name, bytes32 _lastName, bytes32 _idNumber, bytes32 _idNumberNew) public {
        uint index = idToPersonIndex[_idNumber];
    ...
    function enableDisablePerson(bytes32 _idNumber, bool state) public {
        uint index = idToPersonIndex[_idNumber];
        if (state == true) {
